...
export function CareRecordPanel({
  plantId,
  plantName,
  plants = [],
  variant = "desktop"
  ...
  const selectedPlantName = plants.find((plant) => plant.id === selectedPlantId)?.name ?? plantName;
  const canSelectPlant = plants.length > 1;
  const refresh = useSyncRefresh({ plantId: selectedPlantId, resources: ["care_records"] });
  const records = useAsync(
    () => api.listCareRecords(selectedPlantId, CARE_RECORD_FETCH_LIMIT),
    [selectedPlantId, refresh]
  );
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      await api.createCareRecord(selectedPlantId, {
        type: activeType,
      ...
      toast.show({
        ...
        description: `${selectedPlantName} 的养护记录又添一笔。`,
        tone: "success",
        durationMs: 2600
      });
    } catch (error) {
    ...
    <Card className="flex flex-col gap-md !p-md md:!p-lg">
      ...
      <div className="flex flex-col gap-sm rounded-md bg-surface-container-low/70 ring-1 ring-surface-container-highest/40 p-sm md:p-md">
        {canSelectPlant ? (
          <PlantSwitcher
            plants={plants}
            activeId={selectedPlantId}
            ariaLabel="选择本次养护的植物"
            onSwitch={selectPlant}
      ...
      {records.loading && !records.data ? (
        ...
        <Empty
          icon="eco"
          title="还没有养护记录"
          description={`给 ${selectedPlantName} 浇水、施肥、修剪后点一下上面的按钮，这里会留下照料它的足迹。`}
        />
      ) : (
