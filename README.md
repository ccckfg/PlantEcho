...
LLM 只负责把这些结构化事实润色成植物口吻的自然语言。**它不能编造传感器读数，不能假装记得没有记录的事。**
...
| 触发 | 事实 | 它会说 |
|---|---|---|
| 💧 缺水 | 土壤低于建议下限 | 「我有点渴了，可以来点水吗？」 |
| 📡 离线 | 超阈值无新读数 · 冷却 12h | 「我暂时听不到自己的传感器了。」 |
| 🌧️ 降雨 | 实时天气含雨 · 冷却 6h | 「外面像要下雨，出门记得带伞。」 |
| ⏰ 到期提醒 | 你托它记的事 · 最长 30 天 | 「你让我提醒你：该浇水啦。」 |
...
| `LLM_API_URL` / `LLM_API_KEY` / `LLM_MODEL_ID` | OpenAI 兼容 LLM，用于聊天与记忆生成 |
| `EMBEDDING_*` | 可选 embedding 服务；留空时复用 LLM 配置 |
| `RERANK_*` | 可选 rerank 服务（如 `Qwen/Qwen3-Reranker-8B`） |
| `WeatherKey` / `WeatherUrl` | 和风天气 / QWeather 配置 |
| `PROACTIVE_ENABLED` | 主动发言引擎开关 |

> LLM / embedding / rerank 均为**可选**：缺失时系统回退到规则与模板，并对用户诚实标注「回复来自本地预设」。
