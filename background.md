...
- `archive`：不参与构建的本地历史资料（已在 Git 与 Docker 构建上下文中忽略）。
  - `references`：AgentGal 实验仓库与 Verdant Echo 设计稿原件。
  - `presentations`：汇报文案、PPT 生成源文件与期中汇报材料。
  - `drafts`：临时文档备份与草稿。
  - `hardware-materials`：面包板版本 AI 小智教程、固件与刷机资料。
- `background.md`：本文件。
